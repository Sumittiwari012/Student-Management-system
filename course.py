...
def courseCreate(cour_id, course_name):
    csv_reader = []
    ...
    for i in range(0, len(csv_reader)):
        if(csv_reader[i][0] == cour_id):
            ...
            return
    print("Enter the batches in which course is included: ")
    students = []
    while(True):
        bat_id = input("Enter the batch ID (to stop enter STOP): ")
        if(bat_id.upper() == "STOP"):
            break
        else:
            check = 0
            for i in range(0, len(csv_reader)):
                ...
                if(csv_reader[i][0] == bat_id):
                    check = 1
                    if(csv_reader[i][3] == ""):
                        csv_reader[i][3] = csv_reader[i][3] + cour_id
                    else:
                        csv_reader[i][3] = csv_reader[i][3] + ":" + cour_id
    ...
    for a in students:
        temp[a] = 0
    data = [cour_id, course_name, json.dumps(temp)]
    with open("course.csv", "a", newline = "\n") as f:
        csv_writer = csv.writer(f)
        csv_writer.writerow(data)

def checkPerformance(cour_id):
    csv_reader = []
    data = []
    with open("course.csv", "r", newline = "\n") as f:
        csv_reader = list(csv.reader(f, delimiter=","))
